...
//        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + TABLE_NAME + "(ID INTEGER UNIQUE, product_name TEXT UNIQUE, product_url TEXT, product_liked INTEGER NOT NULL DEFAULT 0 CHECK(product_liked IN (0,1)))");
    }
    ...
    public boolean insertData(String product_name, String url, boolean liked, int productId)
    {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public Cursor getAllFav(){
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("SELECT * FROM " + TABLE_NAME +" WHERE product_liked = 1 ORDER BY ID ASC", null);
        return res;
    }
